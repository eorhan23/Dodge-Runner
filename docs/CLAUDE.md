...
- **`docs/PRD.md`** — MVP kapsamı: hangi özellikler var. (`docs/PRD_v0.md` bu dosyanın ilk taslak halidir, referans/arşiv amaçlıdır.)
- **`docs/PRD_v1.md`, `docs/PRD_v2.md`** — MVP sonrası backlog fikirleri (görsel/ses katmanı, karakter özelleştirme, power-up, çoklu seviye, online skor tablosu, mobil sürüm vb.). **Bunları MVP fazında uygulama** — yalnızca kullanıcı açıkça isterse veya `tasks/Tasks.md`'deki "Backlog — MVP Dışı" bölümüne taşındığında ele al.
